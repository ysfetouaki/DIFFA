  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter excursions by location (CIRCUITS)
        const circuitsActivities = excursions
          .filter((excursion: any) => {
            const location = excursion.location?.toLowerCase() || '';
            const section = excursion.section?.toLowerCase() || '';
            
            return location.includes('circuits') || 
                   location.includes('circuit') || 
                   section.includes('circuits') ||
                   section.includes('circuit');
          })
          .map((excursion: any) => ({
            id: excursion.id,
            name: excursion.name,
        ...
        <section className="relative h-[400px] md:h-[500px] w-full overflow-hidden">
          ...
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
            <div className="text-center text-white px-4">
              <h1 className="text-4xl md:text-6xl font-display font-light mb-4">{t('circuitsPage.hero.title')}</h1>
