    ...
    const fetchData = async () => {
      try {
        ...
        if (settingsResponse.ok) {
          ...
          const agadirSetting = settings.find((s: any) => s.section === 'agadir');
          if (agadirSetting) {
            setShowPrice(agadirSetting.showPrice);
          }
        }
        
        // Filter excursions by location (AGADIR)
        const agadirActivities = excursions
          .filter((excursion: any) => {
            const location = excursion.location?.toLowerCase() || '';
            const section = excursion.section?.toLowerCase() || '';
            
            return location.includes('agadir') || section.includes('agadir');
          })
          .map((excursion: any) => {
        ...
        setActivities(agadirActivities);
      } catch (err) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24">
        ...
        <section className="relative h-[400px] md:h-[500px] w-full overflow-hidden">
          <Image
            src="https://slelguoygbfzlpylpxfs.supabase.co/storage/v1/object/public/project-uploads/9954d743-8735-4b3b-868c-4bb1cfd3cbf8/generated_images/beautiful-agadir-beach-coastline-morocco-08392506-20251124020921.jpg"
            alt={t('agadirPage.hero.title')}
            fill
          ...
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
            <div className="text-center text-white px-4">
              <h1 className="text-4xl md:text-6xl font-display font-light mb-4">{t('agadirPage.hero.title')}</h1>
              <p className="text-lg md:text-xl font-body max-w-2xl mx-auto">
                {t('agadirPage.hero.description')}
              </p>
            </div>
          </div>
        </section>
